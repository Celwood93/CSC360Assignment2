...
void * customer_entry(void * cus_info){
	...
	//this makes it so the clerk will broadcast to the queue it is going to serve, and all of them ddecerement by 1 and see if they can go
	do:
		pthread_cond_wait(/* cond_var of selected queue */, /* mutexLock of selected queue */);
		//decrement position in queue
	while(position in queue is not )
